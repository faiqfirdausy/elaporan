...
class TahunanController extends Controller
{
    ...
    {
        $data['laporan'] = Transaksi::where('id_laporan','>',9)->where('id_laporan','<',12)->get();
        return view('tahunan.index', $data);
    }

    public function create()
    {
        $data['laporan'] = Laporan::where('id_jenis_laporan',4)->get();
        return view('tahunan.create',$data);
    }
    public function ubah(Request $request, $id)
    {
        $data['laporan'] = Transaksi::findOrFail($id);
        $data['lap'] = Laporan::where('id_jenis_laporan',4)->get();

        return view('tahunan.ubah',$data);
       ...
       // dd($request);
                ...
                if($checkpath != null)
                {
                     ...
                     if ($request->hasFile('file_laporan'))
                    {
                        ...
                        Storage::disk('local')->putFileAs('file_laporan/tahunan', $file, $filename);
                        $fileLaporan = 'file_laporan/tahunan/'.$filename;
                    }
